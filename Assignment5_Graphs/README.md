...
Our graph node_t differs however, one of its fields will be a list that will contain the nodes we pointing to (out_neighbors), and the nodes that point to us (in_neighbors);
...
What we are left to do is our <code> graph_t </code> structure? We only need to store all of our nodes, and we can store them in a doubly linked list.
...
Notice that the neighbors in the graph node (and the nodes in the graph) are stored in a ```dll```. Our ```dll``` from Assignment 3 stores integers. Therefore we need need to change the data type in the `dll` node from ```int``` to ```void*```, since we are storing pointers to graph nodes. Notice how we didn't change the data to ```graph_node_t*``` but rather to ```void*```. This enables us to store any type of pointer in the ```dll```, and not just ```graph_node_t*```. This is how C data structures deal with being able to store different data types. If this wasn't the case we would need a different ```dll``` for every type.
...
Now we can reuse all of the DLL functionallity and the implementation of our graph becomes easier.
...
The structure of the code has already been provided for you. You need to paste your DLL code for each function from Assignment 3 into the ```my_dll.h``` header file. Note that the signatures have already been updated for you.  

As this is a longer assignment than usual please compile very often after small changes. Make sure that every small chnage that you write works by compiling and **testing** it, before moving on to another change. Writing a lot of code without compiling it or even testing it especially for this assignment is very likly to make you unsuccessful. 

We did not provide you with a working DLL because there are students that might choose to redo this assignment. 
...
- 100% Part 1 correct graph implementation
